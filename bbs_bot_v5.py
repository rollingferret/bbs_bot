...
class BotConfiguration:
    ...
    POLL_POPUP: float = 0.5           # Throttled scan for center popups
    POLL_RECOVERY: float = 0.5        # Baseline anchor scan
    POLL_RUNNING: float = 0.5         # End-of-quest check (Snappier rewards)
...
class BBSBot:
    ...
    # --- THE V4 STEALTH CORE ---
    def smart_click(self, target: Union[str, pyscreeze.Box], description: str, verify_key: Optional[str] = None, target_state: Optional[str] = None, wait_for_appearance: bool = False, custom_delay: Optional[Union[float, Tuple[float, float]]] = None, confidence: Optional[float] = None, region: Optional[Tuple[int, int, int, int]] = None) -> bool:
        """
        V3-Style Smart Click: Locate -> Click -> Refocus.
        No universal soak. Lightning fast, state-machine driven.
        """
        conf = confidence or self.config.CONF_NORMAL
        ...
        # 2. Human Cognitive Pacing (NO Universal Soak)
        ...
        # 7. Rapid Verification Window (V5.1 Stability Patch)
        if success and verify_key:
            v_start = time.time()
            while time.time() - v_start < 1.0: # 1s max wait for UI response
                found_v = self.find_image(verify_key, confidence=self.config.CONF_VERIFY_ACTION)
                # Success Condition: (Wanted appearance AND found) OR (Wanted disappearance AND not found)
                if (wait_for_appearance and found_v) or (not wait_for_appearance and not found_v):
                    if target_state: self.transition_to(target_state)
                    return True
                time.sleep(self.config.POLL_UI_VERIFY)
            
            # If we were verifying and timed out, do NOT transition. 
            # Let the main loop try again on the next tick.
            return False

        # 8. Instant Transition (V3 Style)
        # Only happens if no verify_key was provided to gate the transition
    ...
    def handle_global_popups(self) -> bool:
        # V5 Optimization: Throttled scanning. Only check for popups periodically.
        now = time.time()
        ...
        for key in popups:
            ...
            if self.find_image(key, confidence=conf, region=reg):
                ...
                self.smart_click(key, f"dismiss {key}", verify_key=key, custom_delay=self.config.DELAY_POPUP, confidence=conf, region=reg)
            ...
            if self.smart_click("enter_room_button", "enter room list"):
                ...
                while time.time() - start_load < self.config.TIMEOUT_ROOM_LIST_LOAD:
                    if self.find_image("auto") or self.find_image("search_again"):
                        # V5.6: Mandatory soak to allow the list to finish expanding/sliding
    ...
    def handle_ready(self) -> None:
        ...
        if self.find_stable_image("ready", confidence=self.config.CONF_READY, frames=3):
            # V5.1: Small explicit soak for the lobby button
            time.sleep(self.config.DELAY_READY)
            if self.smart_click("ready", "ready button", verify_key="ready", target_state="CHECK_RUN_START"):
                return
        else:
    ...
    def handle_check_run_start(self) -> None:
        ...
        if self.find_image("ingame_auto_off", confidence=self.config.AUTO_MATCH_CONFIDENCE):
            if self.config.MANAGE_INGAME_AUTO:
                if self.smart_click("ingame_auto_off", "enable auto", verify_key="ingame_auto_on", target_state="RUNNING", wait_for_appearance=True, confidence=self.config.AUTO_MATCH_CONFIDENCE):
                    return
            self.transition_to("RUNNING"); return
    ...
    def handle_game_startup(self) -> None:
        for key in ["game_start", "close_news", "coop_1", "coop_2"]:
            ...
            if self.find_image(key, confidence=conf):
                ...
                # V5.1: Profile-driven delays now handle this tempo
                return 
