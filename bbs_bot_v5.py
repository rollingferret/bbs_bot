...
class BBSBot:
    ...
    def handle_ready(self) -> None:
        ...
        if self.find_image(
            "closed_room_coop_quest_menu", confidence=self.config.CONF_NORMAL
        ):
        ...
        if self.find_image(
            "close", confidence=self.config.CONF_NORMAL
        ) or self.find_image("unavailable_close", confidence=self.config.CONF_NORMAL):
    ...
    def handle_check_run_start(self) -> None:
        ...
        if self.find_image(
            ...
            if self.smart_click(
                "closed_room_coop_quest_menu", "room closed before start"
            ):
                self.transition_to("MENU")
                return
    ...
    def ensure_window_ready(self) -> None:
        try:
            ...
            if (
                self.win_id != old_wid
                or now - getattr(self, "_last_property_sync", 0) > 5.0
            ):
                self.setup_window_properties()
