...
class BotConfiguration:
    ...
    def __post_init__(self):
        self.CIRCADIAN_PROFILES = {
            "SHIKAI_MAX": {
                "DELAY_COGNITIVE": (0.28, 0.08), "DELAY_SNIPE": 0.20, "DELAY_TRANSITION": 0.7,
                "DELAY_POPUP": 1.0, "DELAY_READY": 0.20, "WAIT_ROOM_LOAD": 0.4,
                "WAIT_SEARCH_AGAIN": 1.0, "WAIT_LOBBY_READY": 0.5, "WAIT_POST_RETRY": 2.0,
                "DURATION_MINS": (45, 90) # Sweaty grinding
            },
            "SHIKAI_NORMAL": {
                "DELAY_COGNITIVE": (0.35, 0.12), "DELAY_SNIPE": 0.35, "DELAY_TRANSITION": 1.2,
                "DELAY_POPUP": 1.5, "DELAY_READY": 0.40, "WAIT_ROOM_LOAD": 0.8,
                "WAIT_SEARCH_AGAIN": 1.8, "WAIT_LOBBY_READY": 1.0, "WAIT_POST_RETRY": 3.5,
...
class BBSBot:
    ...
    # --- ACTIONS ---
    def smart_click(self, box: pyscreeze.Box, description: str = "element", custom_delay: Optional[Union[float, Tuple[float, float]]] = None) -> bool:
        ...
        current_focus = None
        try:
            wid_raw = subprocess.check_output(["xdotool", "getactivewindow"], text=True, stderr=subprocess.DEVNULL).strip()
            if wid_raw: current_focus = wid_raw
        except: pass
        ...
        if success and current_focus and current_focus != self.win_id:
            ...
            try: subprocess.run(["xdotool", "windowactivate", "--sync", current_focus], check=False, stderr=subprocess.DEVNULL)
            except: pass
    ...
    def smart_click_and_verify(self, box: pyscreeze.Box, description: str, verify_key: str, timeout: float = 5.0, target_state: Optional[str] = None) -> bool:
    ...
    def handle_global_popups(self) -> bool:
        ...
        for key in popups:
            ...
            if box:
                ...
                self.transition_to("MENU"); return True
        return False
    ...
    def handle_enter_room_list(self) -> None:
        ...
        if ebit:
            ...
            recognition_delay = self.config.DELAY_TRANSITION * 1.2 # Snappier 0.8s recognition
    ...
    def handle_scan_rooms(self) -> None:
        ...
        if autos:
            ...
            for auto, rule in valid:
                ...
                self.smart_click(pyscreeze.Box(px-20, py-10, 40, 20), "snatch room", custom_delay=self.config.DELAY_SNIPE)
                
                start_v = time.time()
                while time.time() - start_v < 6.0:
                    rbox = self.find_stable_image("ready", confidence=self.config.CONF_READY)
                    if rbox:
                        time.sleep(self.config.WAIT_LOBBY_READY)
                        if self.smart_click(rbox, "snap ready", custom_delay=self.config.DELAY_READY):
                            vv_start = time.time()
                    ...
                    if self.find_image("closed_room_coop_quest_menu", confidence=0.9): 
                        ...
                        if box: self.smart_click(box, "close room full"); break # Next room
                    if self.find_image("close", confidence=0.9): 
                        box = self.find_image("close")
                        if box: self.smart_click(box, "close unavailable"); break # Next room
                    time.sleep(self.config.POLL_UI_VERIFY)
        ...
        for a in self.dedupe_autos(autos):
            ...
            for r in rules:
                ...
                if ry > ay:
                    ...
                    if d < min_d and d < self.config.MAX_RULE_DISTANCE: min_d, best_r = d, r
    ...
    def handle_ready(self) -> None:
        ...
        if rbox:
            if self.smart_click(rbox, "ready button", custom_delay=self.config.DELAY_READY):
                v_start = time.time()
                while time.time() - v_start < 5.0:
                    if self.find_image("retire"): self.transition_to("CHECK_RUN_START"); return
                    time.sleep(self.config.POLL_UI_VERIFY)
    ...
    def handle_check_run_start(self) -> None:
        ...
        if self.find_image("ingame_auto_on", confidence=self.config.CONF_HIGH, region=reg):
            self.transition_to("RUNNING"); return
        offbox = self.find_image("ingame_auto_off", confidence=self.config.AUTO_MATCH_CONFIDENCE, region=reg)
        if offbox:
    ...
    def handle_finish(self) -> None:
        ...
        if tap1_box:
            if self.smart_click(tap1_box, "reward tap1"):
                time.sleep(self.config.DELAY_TAP); self.quest_watchdog = time.time()
            return
        tap2_box = self.find_image("tap2")
        if tap2_box:
            if self.smart_click(tap2_box, "reward tap2"):
                time.sleep(self.config.DELAY_TRANSITION); self.quest_watchdog = time.time()
            return
    ...
    def retire_from_quest(self) -> None:
        ...
        if rbox:
            self.smart_click(rbox, "retire"); time.sleep(self.config.WAIT_RETIRE_STEP)
            ok = self.find_image("okay")
            if ok: 
                self.smart_click(ok, "confirm"); time.sleep(self.config.WAIT_RETIRE_STEP)
                final = self.find_image("closed_room_coop_quest_menu")
                if final: self.smart_click(final, "final confirm")
    ...
    def setup_window_properties(self) -> None:
        if self.win_id and self.config.USE_WMCTRL_ALWAYS_ON_TOP:
            subprocess.run(["wmctrl", "-i", "-r", self.win_id, "-b", "add,sticky,above,skip_taskbar,skip_pager"], check=False, stderr=subprocess.DEVNULL)
