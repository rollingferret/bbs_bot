...
class BBSBot:
    ...
    def check_session_limit(self):
        ...
        if elapsed_hours >= self.config.SESSION_MAX_HOURS:
            ...
            try: subprocess.run(["pkill", "-f", "BleachBraveSouls.exe"], check=False)
            except: pass
            sys.exit(0)
    ...
    def find_image(self, template_key, confidence=None, region=None):
        ...
        try:
            ...
            logger.debug(f"Vision miss ({template_key}): {e}")
            return None
    ...
    def find_all_images(self, template_key, confidence=None, region=None):
        ...
        try:
            ...
            logger.debug(f"Vision all miss ({template_key}): {e}")
            return []
    ...
    def take_debug_screenshot(self, tag):
        ...
        try: pyautogui.screenshot(region=self.region).save(path)
        except: pass
    ...
    def get_game_region(self):
        try:
            ...
            else: raise GameWindowNotFoundError("Game process not found.")
    ...
    def setup_window_properties(self):
        if self.config.USE_WMCTRL_ALWAYS_ON_TOP:
            try: subprocess.run(["wmctrl", "-r", self.config.GAME_WINDOW_TITLE, "-b", "add,sticky,above"], check=True)
            except: pass
    ...
    def smart_click(self, box, description="element", is_overlay=False):
        ...
        if is_overlay: sigma_x = sigma_y = 100
        else: sigma_x, sigma_y = box.width / 4, box.height / 4
        ...
        current_window = None
        try: current_window = subprocess.check_output(["xdotool", "getactivewindow"], text=True).strip()
        except: pass
        success = self.send_x11_click(click_x, click_y)
        if success and current_window:
            time.sleep(0.01)
            try: subprocess.run(["xdotool", "windowactivate", "--sync", current_window], check=False)
            except: pass
    ...
    def recover_game(self):
        ...
        while time.time() - start_wait < 120:
            if self.find_image("game_start"):
                ...
                break
            time.sleep(5)
        self.get_game_region(); self.setup_window_properties()
    ...
    def handle_check_run_start(self):
        ...
        if time.time() - self.last_state_change_time > self.config.TIMEOUT_RUN_START:
            logger.error("Run start timeout (300s). Retiring.")
            ...
            if retire_box:
                ...
                if okay_box: 
                    ...
                    self.fatigue_delay(self.config.WAIT_RETIRE_STEP)
                    final_box = self.find_image("closed_room_coop_quest_menu")
                    if final_box: self.smart_click(final_box, "final retire confirm")
            self.transition_to("MENU")
        time.sleep(2.0)
    ...
    def handle_running(self):
        if self.config.MANAGE_INGAME_AUTO:
            ...
            if offbox: self.smart_click(offbox, "enable auto")
        tbox = self.find_image("tap1")
        if tbox:
            self.transition_to("FINISH"); return
        cbox = self.find_image("close")
        if cbox: self.smart_click(cbox, "close disconnect")
    ...
    def handle_finish(self):
        ...
        if rt:
            ...
            self.run_count += 1
            if self.run_count >= self.next_distraction_run: self.transition_to("DISTRACTION")
    ...
    def handle_game_startup(self):
        for key in ["game_start", "close_news", "coop_1", "coop_2"]:
            ...
            if box: self.smart_click(box, f"startup {key}")
    ...
    def handle_recovery(self):
        ...
        if time.time() - self.last_state_change_time > self.config.TIMEOUT_STUCK:
            ...
            recovery_templates = [
                ("RUNNING", "ingame_auto_on"), ("RUNNING", "ingame_auto_off"),
                ("CHECK_RUN_START", "retire"), ("FINISH", "tap1"),
                ("FINISH", "tap2"), ("FINISH", "retry"), ("READY", "ready"),
    ...
    def run(self):
        ...
        if "--debug-screenshots" in sys.argv: self.config.TAKE_DEBUG_SCREENSHOTS = True
        try:
